# PLAN.v2.md — Group-aware refactor (Round 3 merged)

**Pipeline**: /plan-hardened
**Status**: 2 NEEDS_USER decisions pending in OPEN-AMBIGUITIES.md
**Round 2 input**: 22 Codex issues — 16 VALID merged, 0 INVALID rejected, 6 NEEDS_USER escalated (collapsed into 2 questions)
...
- `app/execution/engine.py` (linee 211-249, focus 222-225) — il loop diventa: per ogni gruppo di mercati con stesso `event_id` e stesso dominio, costruire `valuations_by_market_id` per il gruppo e chiamare `strategy.evaluate(group, valuations_by_id)`. Mantenere il dedup `open_position_token_ids` e il filtro `exited_market_ids`. La riga 222 (`get_for_domain(market.category.value)`) si sposta a livello gruppo (vedere [TBD-Q2] per gruppi multi-category). **Aggiungere step 5d: arbiter event-level cross-strategy**, vedere R2-11. <!-- R2-09, R2-10, R2-11, R2-21 merged -->
...
### File con scope expansion pendente [TBD-Q1]

I seguenti file sono toccati dal piano draft ma NON sono nello scope autorizzato originale. La loro inclusione è oggetto di **Q1** in OPEN-AMBIGUITIES.md. Le sezioni dipendenti sono marcate `[TBD-Q1]`.

- `app/risk/manager.py` [TBD-Q1] — necessario per PR-J (event-level exposure cap). <!-- R2-01, R2-16, R2-17 escalated -->
- `app/core/dependencies.py` [TBD-Q1] — necessario per registrare strategie wrapped/un-wrapped lungo PR-B..PR-K. <!-- R2-02, R3-01 escalated -->
- `app/core/yaml_config.py` [TBD-Q1] — necessario per esporre `risk.max_event_exposure_pct`. <!-- R2-03 escalated -->
- `config/config.example.yaml` [TBD-Q1] — necessario per default config visibile. <!-- R2-03 escalated -->

Se Q1 risolve "split PR-J into separate plan + register strategies elsewhere", questi file escono da scope e PR-J diventa parte di un plan distinto. Se Q1 risolve "approve scope expansion", restano in scope con i constraint aggiuntivi documentati in PR-J revised.
...
- `tests/test_risk/test_event_exposure.py` [TBD-Q1] — gate `max_event_exposure_pct`, accumulo per `event_id`, reset su `record_close`, **persistence cross-restart e atomic reserve-and-record concurrency** (R2-16, R2-17).
...
- `tests/test_execution/test_engine.py` (modify, non new) — aggiungere casi: `test_arbiter_picks_one_winner_per_event_across_strategies` (R2-11), `test_drops_signal_with_market_id_outside_group` (R2-21), `test_top_k_cap_applied_before_vae_for_large_event` (R2-10).
...
- `tests/test_execution/test_engine.py` — aggiornare i test che mockano `strategy.evaluate(market, valuation)` alla nuova firma. Aggiungere test che engine raggruppa per `event_id`, che chiama `evaluate` una volta per gruppo, **arbiter event-level** (R2-11), **out-of-group market_id rejection** (R2-21), **top-K cap pre-VAE** (R2-10).
...
2. **`app/strategies/_legacy_adapter.py`** (nuovo) — <!-- R2-12, R2-13 merged -->
   ...
   2b. **Caso speciale arbitrage**: in `app/core/dependencies.py` (vedere step 4 [TBD-Q1]), wrap arbitrage con `LegacyStrategyAdapter(arbitrage_strategy, aggregate_lists=True)`. **Alternativa robusta**: migrare arbitrage direttamente in PR-B (anticipando PR-I). Decisione drafter: usare `aggregate_lists=True` flag. Se in test emergono regressioni, anticipare migration in PR-B.

3. **`app/strategies/registry.py`** — aggiungere `wrap_legacy(strategy, *, aggregate_lists=False) -> BaseStrategy` factory.

4. **`app/core/dependencies.py`** [TBD-Q1] — al momento di registrare le 7 strategie, wrappare 6 con `LegacyStrategyAdapter(strategy)` e arbitrage con `LegacyStrategyAdapter(arbitrage, aggregate_lists=True)`. (Quando una strategia sarà migrata, si registrerà direttamente senza wrap.)
...
5. **`app/services/market_service.py`** — implementare `get_grouped_markets()` reale: <!-- R2-05, R2-06, R2-09, R2-10, R2-20 merged -->
   ...
   - **Top-K cap PRE-VAE (R2-10)**: dentro `get_grouped_markets()`, dopo grouping, applicare cap `top_K=20` per gruppo (configurabile via [TBD-Q1] `markets.max_per_event` se config in scope; altrimenti hardcoded constant in `market_service.py`). Ordering: `volume_24h` desc, ties → `id` asc. Markets scartati sono LOGGED ma NON valutati. R3-03 caveat: l'engine deve usare il dict `market_by_id` costruito DOPO il top-K cap, non quello pre-cap.
...
6. **`app/execution/engine.py`** (linee 211-249) — refactor del loop: <!-- R2-09, R2-10, R2-11, R2-21 merged -->
   ...
   - Determinare dominio: se tutti i markets del gruppo hanno la stessa `category`, usarla; altrimenti applicare la policy [TBD-Q2] (R2-18).
   ...
   - `result_signals = await strategy.evaluate(group, valuations_subset, knowledge=...)` dove `knowledge` è quella del market più liquido del gruppo (R2-14 default; PR-F implementa il fetch concreto).
   - **Step 5b-validate (R2-21)**: per ogni `sig` in `result_signals`, se `sig.market_id not in {m.id for m in group}` → log `invalid_strategy_signal` con strategy name, market_id, group event_id e SCARTA il signal. Non aggiungerlo a `signal_market_pairs`.
   ...
   - **Step 5d — Event-level cross-strategy arbiter (R2-11)**: dopo la priority sort esistente (linee 251-264), prima di andare in execution loop, applicare:
     ...
     - Group `non_arb_signals` by `mkt.event_id`. Per ogni event_id con > 1 signal, keep only the highest priority (la sort esistente ha già definito l'ordering — pick first per event_id).
...
3. **`app/core/dependencies.py`** [TBD-Q1] — registrare `ValueEdgeStrategy()` SENZA wrap.
...
2. `app/core/dependencies.py` [TBD-Q1] — un-wrap.
...
2. `app/core/dependencies.py` [TBD-Q1] — un-wrap (rimuove `aggregate_lists=True` adapter).
...
### PR-J — Risk: event-level exposure cap [TBD-Q1]

**Status**: questa PR dipende interamente dalla risoluzione di Q1. Se Q1 → "split out", PR-J esce da questo plan. Se Q1 → "approve scope", PR-J resta con i seguenti requirement aggiuntivi.

1. `app/risk/manager.py` [TBD-Q1] — implementare `_event_exposure: dict[str, float]`. <!-- R2-16, R2-17 merged conditionally -->
   - **Persistence (R2-16)**: aggiungere `_token_to_event: dict[str, str]` mantenuto in sync con `_current_positions`. Il TradeStore (e/o RiskKB) deve persistere `event_id` accanto al token_id per il restart. `restore_from_store()` deve ricostruire sia `_event_exposure` sia `_token_to_event`.
   - **Atomic reserve-and-record (R2-17)**: aggiungere `asyncio.Lock` su `RiskManager`. La sequenza `check_order` → `record_fill` deve essere wrapping in un singolo `async with self._lock:` block. Esposto via nuovo metodo `reserve_and_record(signal, price, size_eur, event_id, time_horizon) -> RiskCheck` che engine chiama al posto delle due call separate. Test: due tick concorrenti per stesso event_id non possono entrambi superare il cap.
2. `app/execution/engine.py` — passare `mkt.event_id` (preso da `market_by_id[sig.market_id].event_id`, NO mutazione di `Signal`). `restore_from_store()` deve restaurare event exposure correttamente.
3. `app/core/yaml_config.py` [TBD-Q1] + `config/config.example.yaml` [TBD-Q1] — `risk.max_event_exposure_pct: 8.0` (default; A3 tracked as informativa) + `markets.max_per_event: 20` (top-K cap, R2-10).
4. `tests/test_risk/test_event_exposure.py` (nuovo) — gate enforce, accumulo per event_id, reset su close, **persistence cross-restart**, **atomic concurrency** (asyncio.gather di 2 check_order su stesso event_id, solo uno passa).
5. `tests/test_execution/test_engine.py` — engine passa event_id correttamente; restore_from_store riempie event exposure.
6. Verifica.
...
5. `app/core/dependencies.py` [TBD-Q1] — finalize registration sans wrappers. <!-- R3-01 -->
...
### A2 — Mercati di un evento con `category` differenti → [TBD-Q2]

Vedere **Q2** in OPEN-AMBIGUITIES.md. Il piano marca `[TBD-Q2]` le sezioni dipendenti.

### A3 — Default `max_event_exposure_pct` (8% di equity)

Default 8% confermato dal drafter come ragionevole (50%/8% ≈ 6 eventi pieni). Backtest validation è follow-up post-merge, non parte di questo plan. Se Q1 → "split out", questa decisione si sposta nel plan derivato.
...
**RESOLVED via R2-10**: top_K=20 hardcoded come constant in `market_service.py` per default. Configurabile via `markets.max_per_event` SE Q1 approva scope expansion su `config/config.example.yaml`. Cap applicato PRIMA di `assess_batch`, non dopo.
...
**RESOLVED**: runtime lookup via `market_by_id[sig.market_id].event_id`. Constraint aggiunto: `market_by_id` deve essere built dal pool POST-top-K-cap, vedere R3-03 sotto.

### A12 — Domain split deterministica per gruppi multi-domain

**SUPERSEDED by R2-18 → [TBD-Q2]**. Vedere Q2.

### R3-01 (drafter-added) — Tracking `dependencies.py` modifications across PR-A..PR-K

PR-K cleanup re-finalizza `app/core/dependencies.py`. Tutti i tocchi a `dependencies.py` (PR-B step 4, PR-C..PR-I un-wrap, PR-K finalize) ricadono sotto Q1. Se Q1 → "split out", il wrapping/un-wrapping deve avvenire in `registry.py` (vedere R2-02 fix_proposed) e PR-K finalize si limita a registry.
...
### A13 (renumbered, was implicit) — `last_grouping_incomplete` flag lifecycle
...
9. **L'engine non va riscritto in altri punti**: solo step 5 del tick cambia + nuovo step 5d arbiter (R2-11). Step 1-4 e 6 invariati.
...
14. **`config.example.yaml` è la single source of truth** per default config (se in scope dopo Q1).
15. **Numero di test cumulativo**: ~870 oggi; refactor aggiunge ~40-50 (gruppi, adapter, event_id, exposure cap, fail-closed, top-K, arbiter, validation), modifica ~50-80. Tot stimato post-refactor: ~910-930.
16. **Top-K per evento**: 20 markets (configurabile pendente Q1).
...
| PR-J [TBD-Q1] | `pytest tests/test_risk/test_event_exposure.py tests/test_execution/test_engine.py -v` poi `pytest -q` |
...
### Smoke manuale (PR-J [TBD-Q1] e PR-K)
...
3. Verificare nei log JSON:
   ...
   - `event_exposure` accumulato in `RiskKB` (PR-J — solo se Q1 approva)
   - `grouping_incomplete=False` (idealmente; se True, /events non disponibile e tick degraded come atteso)
4. Restart del bot: `event_id` deve essere persistito (cache 30min eventi, plus per-position event_id se PR-J in scope) e re-fetchato senza errori.

### Done conditions (intero refactor, post PR-K)

- [ ] Preflight precondition green PRIMA di PR-A
- [ ] Tutti i 870+ test originali verdi + ~40-50 nuovi (totale ~910-930) verdi
...
- [ ] `RiskManager` rispetta `max_event_exposure_pct` con persistence + atomic reserve (PR-J [TBD-Q1])
...
## Note finali per Round 4 (User decisions)

Questo plan ha 2 questions aperte in OPEN-AMBIGUITIES.md:
- **Q1**: scope expansion su `app/risk/manager.py`, `app/core/dependencies.py`, `app/core/yaml_config.py`, `config/config.example.yaml` (collassa R2-01, R2-02, R2-03, R2-16, R2-17, R3-01).
- **Q2**: policy per gruppi multi-category (R2-18, A2, A12).

16 issue Codex sono state risolte inline (VALID merged). 0 issue rejected. 6 issue collassate in 2 question per rispettare il cap di ≤5.

Issue R3- (drafter-found) ora documentate inline:
- R3-01: dependencies.py touches ricadono sotto Q1.
- R3-02: VAE cross_market signal interaction da spot-checkare post-PR-A.
- R3-03: market_by_id deve essere built post top-K cap (constraint aggiunto a engine.py refactor).
